...
.stButton > button:not([key^="obj_"]) {
    background: linear-gradient(135deg, #0071e3, #0051a8) !important;
...
with right_col:
    ...
    for i in range(3):
        ...
        with col1:
            st.markdown(f"""
            <div style="padding: 16px; background: #f5f5f7; border: 1px solid #e5e5e7; border-radius: 12px; 
                       display: flex; flex-direction: column; gap: 4px;">
                <div style="font-weight: 600; color: #1d1d1f; font-size: 15px;">
                    <span style="color: #0071e3; margin-right: 8px; font-weight: 700;">{i+1}.</span>{priority_title}
                </div>
                <div style="font-size: 13px; color: #86868b;">{priority_desc}</div>
            </div>
            """, unsafe_allow_html=True)
